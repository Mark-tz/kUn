...
namespace NameSpaceMarkingPosV2{
	...
	double RECEIVER_MARK_BUFFER = 6.0;//receiver 盯人距离
...
{
    ZSS::ZParamManager::instance()->loadParam(DENY_BALL_MODE,"Defence/DenyBallMode",true);
...
{	
	if (pVision->TheirPlayer(oppNum).Valid() && 0 != oppNum)//存在且车号不为0
	{
		...
		if (false && DefenceInfo::Instance()->getOppPlayerByNum(oppNum)->isTheRole("RReceiver"))//如果对方是receiver
		{		
