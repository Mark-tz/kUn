...
void CDefPos2015::generatePos(const CVisionModule* pVision) {
    ...
    if (leftBack != 0 && rightBack != 0) {
        ...
        // 修改传点坐标太趋近于0 超出函数定义域问题 gjy 2018/3/29
        if (abs(_RdefendTarget.x()) < 1e-10) _RdefendTarget.setX(0);
        if (abs(_RdefendTarget.y()) < 1e-10) _RdefendTarget.setY(0);
        if (abs(_RdefendDir) < 1e-10) _RdefendDir = 0;
