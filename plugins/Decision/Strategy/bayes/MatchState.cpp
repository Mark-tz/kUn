...
CMatchState::CMatchState()
{
    ...
    BAYESLIST = "BayesParams1";
    BAYESPARAM = "Skuba_Attack";

	if (RecordFlag) {
		RecordFile.open(RecordName.c_str(),ios::out);
		if (RecordFile.is_open()) {
			RecordFile<<"# Name.bayes"<<endl;
            RecordFile<<BAYESPARAM.toStdString()<<".bayes"<<endl;
...
void CMatchState::initialize(const COptionModule* pOption, const CVisionModule* pVision)
{
    ...
    BayesReader::Instance()->readBayesParam(BAYESLIST.toStdString());
    _bayesfilter.init(BayesReader::Instance()->getBayesParam(BAYESPARAM.toStdString()));
	if (VERBOSE_MODE) {
        BayesReader::Instance()->printBayesParam(BAYESPARAM.toStdString());
	}
	_initialed = true;
