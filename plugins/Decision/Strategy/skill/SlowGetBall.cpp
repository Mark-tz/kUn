...
void CSlowGetBall::plan(const CVisionModule* pVision)
{
	...
	case S_GETBALL:
		actionCount++;
        setSubTask(PlayerRole::makeItNoneTrajGetBall(robotNum,finalDir,CVector(0,0),flags|PlayerStatus::OUR_BALL_PLACEMENT,stopDist, CMU_TRAJ));
		break;
	case S_GOTOWARD:
		actionCount++;
        setSubTask(PlayerRole::makeItRun(robotNum,x_speed,y_speed,0,PlayerStatus::DRIBBLING|PlayerStatus::OUR_BALL_PLACEMENT));
		break;
	case S_STOP:
        setSubTask(PlayerRole::makeItRun(robotNum,0.0,0.0,0.0,PlayerStatus::DRIBBLING|PlayerStatus::OUR_BALL_PLACEMENT));
		break;
	case S_RUSH:
        setSubTask(PlayerRole::makeItNoneTrajGetBall(robotNum,finalDir,CVector(0,0),flags|PlayerStatus::OUR_BALL_PLACEMENT,-2));
		break;
	default: break;
	}
	//限制actionCount
