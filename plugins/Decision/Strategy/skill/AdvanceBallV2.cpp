...
CAdvanceBallV2::CAdvanceBallV2() {
    ZSS::ZParamManager::instance()->loadParam(WEAK_OPP,"JamAndRush/WeakOpp",true);
    ZSS::ZParamManager::instance()->loadParam(OPP_HAS_BALL_DIST,"JamAndRush/OppHasBallDist",50);
    ZSS::ZParamManager::instance()->loadParam(USE_DRIBBLE_TURN,"JamAndRush/WeakOpp",false);
    ZSS::ZParamManager::instance()->loadParam(USE_DRIBBLE_KICK,"JamAndRush/GentleMode",false);
    ZSS::ZParamManager::instance()->loadParam(USE_ROTATE,"JamAndRush/OppHasBallDist",false);
    ZSS::ZParamManager::instance()->loadParam(USE_INTERCEPT,"JamAndRush/CircleFront",true);
...
//				_state = KICK;
			...
			// ½ØÇò
			//cout << "ballmoving2me:" << ballMoving2Me(pVision, _executor) << " " << "menearball:" << meNearballThanOpp(pVision, _executor) << endl;
		...
		case INTERCEPT:
			if (meLoseBall > 60 || !meNearballThanOpp(pVision, _executor)) {
				_state = GOTO;
				if (DEBUG_ENGINE) { cout << "intercept-->goto1" << endl; }
			}
			if (WorldModel::Instance()->isShootLineBlocked(_executor, 600,6)) {
				_state = GOTO;
				if (DEBUG_ENGINE) { cout << "intercept-->goto2" << endl; }
			}
			//cout << "jumpout:" << CInterceptBallV6::jumpOut() <<endl;
			//if (meHasBall >= meHasBallMaxCnt && !WorldModel::Instance()->isShootLineBlocked(_executor, 600, 6)) {
            if (false){//CInterceptBallV6::jumpOut()) {
				_state = GOTO;
				if (DEBUG_ENGINE) { cout << "jump out!!! intercept-->goto3" << endl; }
			}
			break;
		case NORMAL_PUSH:
			normalPushCnt++;
		...
		case KICK:
			...
			if (meLoseBall>8||isShootBlocked(pVision,_executor)) {
...
bool CAdvanceBallV2::isVisionHasBall(const CVisionModule* pVision, const int vecNumber) {
	const PlayerVisionT& me = pVision->OurPlayer(vecNumber);
    const int oppNumber = BestPlayer::Instance()->getTheirBestPlayer();
    const PlayerVisionT& opp = pVision->TheirPlayer(oppNumber);
	const MobileVisionT& ball = pVision->Ball();
    ...
    double opp2Ball = (ball.Pos() - opp.Pos()).dir();
    ...
    bool isVisionPossession = dirVisionHasBall && distVisionHasBall && closerThanOpp;
