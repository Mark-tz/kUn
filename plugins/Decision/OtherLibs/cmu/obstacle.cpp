...
float obstacle::closest_point(state s, vector2f& nearestPoint) {
    ...
    if (this->type != OBS_RECTANGLE) {
        ...
        double x1 = this->seg_start.x;
        double y1 = this->seg_start.y;
        double x2 = this->seg_end.x;
        double y2 = this->seg_end.y;

        if (fabs(sPos.x() - x1) + fabs(sPos.x() - x2) - fabs(x1 - x2) < EPSILON && fabs(sPos.y() - y1) + fabs(sPos.y() - y2) - fabs(y1 - y2) < EPSILON) {
            return 0;
        }

        CGeoLine l1 = CGeoLine(CGeoPoint(x1, y1), CGeoPoint(x1, y2));
        CGeoLine l2 = CGeoLine(CGeoPoint(x1, y2), CGeoPoint(x2, y2));
        CGeoLine l3 = CGeoLine(CGeoPoint(x2, y2), CGeoPoint(x2, y1));
        CGeoLine l4 = CGeoLine(CGeoPoint(x1, y1), CGeoPoint(x2, y1));
        CGeoPoint p1 = l1.projection(sPos);
        CGeoPoint p2 = l2.projection(sPos);
        CGeoPoint p3 = l3.projection(sPos);
        CGeoPoint p4 = l4.projection(sPos);

        minDist = min(p1.dist(sPos), min(p2.dist(sPos), min(p3.dist(sPos), p4.dist(sPos))));
    }
	return(minDist);
}
...
void obstacles::add_long_circle(vector2f x0, vector2f x1, vector2f v, float r, int mask, bool DRAW_OBSTACLES, ObstacleType type) {
...
void obstacles::add_circle(vector2f x0, vector2f v, float r, int mask, bool DRAW_OBSTACLES, ObstacleType type) {
	if(num >= MAX_OBSTACLES) return;
...
void obstacles::add_rectangle(vector2f x0, vector2f x1, int mask, bool DRAW_OBSTACLES, ObstacleType type) {
	if (num >= MAX_OBSTACLES) return;
    obs[num].type = type; // 矩形
    obs[num].mask = mask;
	obs[num].seg_start = x0;
	obs[num].seg_end = x1;
//    GDebugEngine::Instance()->S
    if(DRAW_OBSTACLES)
