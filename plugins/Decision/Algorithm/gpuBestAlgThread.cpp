...
const int OURPLAYER_NUM = Param::Field::MAX_PLAYER;
const int THEIRPLAYER_NUM = Param::Field::MAX_PLAYER;

#define UN_GPU_PRECISE 10

extern std::mutex* _best_visiondata_copy_mutex;
extern std::mutex* _value_getter_mutex;
extern Semaphore vision_finish_semaphore;
CGeoPoint C_ourPlayer_Geo[Param::Field::MAX_PLAYER];
CGeoPoint C_theirPlayer_Geo[Param::Field::MAX_PLAYER];
...
const double dWeight7 = 80; //射门角度的权值
const double dWeight8 = -100; //接球角度的权值
...
const double endX = 450;
const double startY = -450;
const double endY = 450;
...
void CGPUBestAlgThread::initialize(CVisionModule* pVision) {
    _pVision = pVision;
    // 开启 GPU 计算的线程
    _best_calculation_thread = new std::thread([ = ] {doBestCalculation();});
    return;
}
...
void ZSS_THREAD_FUNCTION CGPUBestAlgThread::doBestCalculation() {
    while (true) {
...
/*射门角度计算                                                          */
/************************************************************************/
inline float C_GetDirRange(const C_blockDirection& D) {
    return (D.leftDist <= D.rightDist ? D.leftDist : D.rightDist) * fabs(D.rightDir - D.leftDir);    //此处暂时加上fabs()，若程序没问题就去掉
}
inline float C_GetDirMiddle(const C_blockDirection& D) {
    return (D.leftDir + D.rightDir) / 2.0;
}
//射门角度计算函数
...
//    vector <CGeoPoint> flatPoint(&flatPointTmp[0], &flatPointTmp[PointNum]);
    ...
    CGeoPoint flatPointTmp4[PointNum];
    for(int i = 0; i < PointNum; i++) {
        flatPointTmp4[i] = CGeoPoint(flatPointTmp[i].x() / 2, flatPointTmp->y() * 5 / 6);
    }
    vector <CGeoPoint> flatPoint(&flatPointTmp4[0], &flatPointTmp4[PointNum]);
    ...
    if (!isChip) {
        ...
        for (i = 0; i < size; i++) {
            ...
            for (j = 1; j <= Param::Field::MAX_PLAYER; j++) {
                ...
                if (_pVision->OurPlayer(number[j]).Valid()) {
                    dist = _pVision->OurPlayer(j).Pos().dist(flatPoint[i]);
                    if (dist < me2PointDist && j != number[4]) { // number[4] 是 Defender?
                        me2PointDist = dist;
                        getBallNum = j;
                    }
                }
            ...
            for (k = 1; k <= Param::Field::MAX_PLAYER; k++) {
                if (_pVision->TheirPlayer(number[k]).Valid()) {
...
//	}
    if (determinValueForChipKick > 0) {
        ...
        cout << chipPower << endl;
        return chipPoint[numRecord];
        qDebug() << chipPoint[numRecord].x();
    } else {
        chipPower = 250;
        if (_pVision->Ball().Pos().y() > 0)
            return CGeoPoint(140, -60);
        else
            return CGeoPoint(140, 60);
    }
}
...
CGeoPoint CGPUBestAlgThread::bestPointForFrontKicks() {
    ...
    CGeoPoint p(Param::Field::PITCH_LENGTH / 2, tmpY*AntiY);
    const double min_Pos_X = 100;
    const double max_Pos_Y = Param::Field::PITCH_WIDTH / 2;
...
CGeoPoint CGPUBestAlgThread::bestPointForBackKicks() {
    ...
    CGeoPoint targetPoint;
    CGeoPoint leftgoal (Param::Field::PITCH_LENGTH / 2, -30 * AntiY);
    CGeoPoint rightgoal(Param::Field::PITCH_LENGTH / 2, 30 * AntiY);
    vector <CGeoPoint> point;
    ...
    if (determinValueForChipKick > 0) {
        ...
        chipPower = ball2ChipPointDist * 1.4; //* 0.7;
        fixTime = -0.0000015498 * chipPower * chipPower + 0.0025344180 * chipPower + 0.2463515283;
        fixBuf = fixTime * 60;
...
void CGPUBestAlgThread::detectModule(bool generatePos, double detectDist, int detectCycle, bool useGoalie) {  //重要模块！
    ...
    const int attackValue = useGoalie ? 7 : 6;
    ...
    if (generatePos) {
        ...
        if (useGoalie) {
            ramdomNumber[7] = rand() % 3 + 7;
            Num[ramdomNumber[7]] = 999;
        }
        for (i = 1; i <= 9; i++) {
        ...
        cout << "number " << number[0] << " " << number[1] << " " << number[2] << " " << number[3] << " " << number[4] << " " << number[5] << endl;
        cout << "ramdomNumber " << ramdomNumber[0] << " " << ramdomNumber[1] << " " << ramdomNumber[2] << " " << ramdomNumber[3] << " " << ramdomNumber[4] << " " << ramdomNumber[5] << endl;
    ...
    for (i = 0; i < attackValue; i++) {
        // cout << number[i] << endl;
        if (_pVision->OurPlayer(number[i]).Valid()) {
            if (isMarkedForDynamicKick(number[i], detectDist) || _pVision->OurPlayer(number[i]).X() < 0 || !number[i]) {
                if (buf[i]) buf[i] = 0;
                state[i] = 0;
            } else {
                buf[i]++;
    ...
    // cout<<buf[0]<<" "<<buf[1]<<" "<<buf[2]<<" "<<buf[3]<<" "<<buf[4]<<" "<<buf[5]<<endl;
    if (jamMode) {
...
void CGPUBestAlgThread::detectDefendKickedModule(int checkAttempts, int defendKickCycle) {
    ...
    const double startDir = (CGeoPoint(0, -Param::Field::PITCH_WIDTH / 2 * antiY) - ball.Pos()).dir();
    const double endDir = (CGeoPoint(Param::Field::PITCH_LENGTH / 2, 0) - ball.Pos()).dir();
    if (_pVision->Cycle() - _cycle2 > Param::Vision::FRAME_RATE * 0.1) {
        ...
        state[6] = 0;
        maxDefendKickbuf = 0;
        assisterPos = CGeoPoint(0, -Param::Field::PITCH_WIDTH / 2 * antiY);
        if (fabs(ball.Y()) < 200) {
            //checkAttempts = 0;
        }
        if (checkAttempts) {
    ...
    if (!detectDefendKickedOverFlag) {
        if (isDefendKickedForDynamicKick(number[6])) {
            buf[6] = 0;
            flatPassAngleFlag[detectDefendKickAttempts] = false;
        } else {
            buf[6]++;
            if (buf[6] > detectdefendKickCycle) {
    ...
    */
    ...
    if (!useOwnPos) {
        ...
        if (CornerKick == indirectKickType) {
            ...
            // TODO ftq
            int prioritySequence[11] = { 999, 7, 2, 3, 5, 4, 8, 6, 9, 10, 1 }; // quyu
            getBallNumber = 999;
            if (ball.Y() >= 0) {
                int temp;
                for (i = 1; i <= 7; i += 3) {
...
//                        openArea[i] = false;
//                    }
//                    openArea[4]=openArea[5]=openArea[6]=true;
                }
                //cout << point.size() << endl;
            ...
            if (openArea[i]) {
                for (j = area[i]._point[2].x() + 20; j < area[i]._point[1].x(); j += 40) {
                    for (k = area[i]._point[0].y() + 20; k < area[i]._point[1].y(); k += 40) {
                        ...
                        if (!Utils::InTheirPenaltyArea(targetPoint, 10) && !Utils::OutOfField(targetPoint)) point.push_back(targetPoint);
                    }
                }
            }
        }
...
void CGPUBestAlgThread::scanModule() {
    ...
    if (!isChipForDynamicKick) {
        ...
        for (i = 0; i < size; i++) {
            ...
            CShootRangeList shootRangeList(_pVision, getBallNumber, point[i]);
            const CValueRange* bestRange = NULL;
            const CValueRangeList& shootRange = shootRangeList.getShootRange();
            if (shootRange.size() > 0) {
                bestRange = shootRange.getMaxRangeWidth();
            ...
            //
            //cout << ball2PointDir << " " << canFlatPassTo << endl;
            minDist = minDist > thresholdForFlatMinDist ? thresholdForFlatMinDist : minDist;
            enemy2MeDist = enemy2MeDist > thresholdForEnemy2MeFlatDist ? thresholdForEnemy2MeFlatDist : enemy2MeDist;
            enemy2PointDist = enemy2PointDist > thresholdForEnemy2PointDist ? thresholdForEnemy2PointDist : enemy2PointDist;
            if (isPassLineBlock || minDist < determinDist3 || enemy2PointDist < determinDist4 || ((enemy2MeDist < determinDist5) && (_pVision->TheirPlayer(enemyNumRecord).Pos().x() > _pVision->OurPlayer(number[getBallNumber]).Pos().x())) || sizeOfDir < minShootDir || ball2PointDist < 150 || determindir > 75 * Param::Math::PI / 180 || !canFlatPassTo) {
//                if (isPassLineBlock){
//                    cout<<"1"<<endl;
//                }
//                else if (minDist < determinDist3){
//                    cout<<"2"<<endl;
//                }
//                else if (enemy2PointDist < determinDist4){
//                    cout<<"3"<<endl;
//                }
//                else if (((enemy2MeDist < determinDist5) && (_pVision->TheirPlayer(enemyNumRecord).Pos().x() > _pVision->OurPlayer(number[getBallNumber]).Pos().x()))){
//                    cout<<"4"<<endl;
//                }
//                else if (sizeOfDir < minShootDir){
//                    cout<<"5"<<endl;
//                }
//                else if (ball2PointDist < 150){
//                    cout<<"6"<<endl;
//                }
//                else if (determindir>75 * Param::Math::PI / 180){
//                    cout<<"7"<<endl;
//                }
//                else if (!canPass()){
//                    cout<<"8"<<endl;
//                }
                currentValue = 0;
                if (allMarkedFlag && canFlatPassTo && !jamMode)
                    currentValue = 999;
                if (Test) {
                    ...
                    if (!canFlatPassTo) cout << "cannot flatpass" << endl;
                }
            } else {
...
void CGPUBestAlgThread::numberReceive(int first, int second, int third, int fourth, int fifth, int sixth, int assister, int goalie) {
    number[0] = first;
    number[1] = second;
    number[2] = third;
    number[3] = fourth;
    ...
    //cout << "number "<<number[0] << " " << number[1] << " " << number[2] << " " << number[3] << " " << number[4] << " " << number[5] << " " << number[6] <<endl;
}
...
bool CGPUBestAlgThread::isMarkedForDynamicKick(int num, double determindist) {
    const MobileVisionT& ball = _pVision->Ball();
    const PlayerVisionT& me = _pVision->OurPlayer(num);
    ...
    for (i = 1; i <= Param::Field::MAX_PLAYER; i++) {
        if (_pVision->TheirPlayer(i).Valid()) {
            ...
            double dirMe2Enemy = (_pVision->TheirPlayer(i).Pos() - _pVision->OurPlayer(num).Pos()).dir();
...
//    double slack;
    ...
    double trobot = predictedTime(me, shootPos);
    double tball;
    if (isChipForDynamicKick) {
        tball = fixTime * 1.55;
        trobot = trobot ;
...
const CGeoRectangle* CGPUBestAlgThread::getRunArea() {
    ...
    for (int i = 1; i <= sizeof(area) / sizeof(area[0]); i++) {
        runArea.push_back(area[i]);
    }
    return area;
}

void CGPUBestAlgThread::generateRunPos(int areaNumber, vector<CGeoPoint>& myPos) {
    CGeoPoint targetPoint;
    int j, k = 0;
    const int antiY = _pVision->Ball().Y() > 0 ? -1 : 1;
    ...
    if (areaNumber < 10) {
        for (j = area[areaNumber]._point[2].x() + 20; j < area[areaNumber]._point[1].x(); j += 40) {
            for (k = area[areaNumber]._point[0].y() + 20; k < area[areaNumber]._point[1].y(); k += 40) {
                targetPoint = CGeoPoint(j, k);
                if (!Utils::InTheirPenaltyArea(targetPoint, 10) && !Utils::OutOfField(targetPoint)) myPos.push_back(targetPoint);
            }
        }
    } else {
        for (int i = 0; i < 10 ; i++) {
