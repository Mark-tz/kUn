...
#define  C_MAX_PLAYER		Param::Field::MAX_PLAYER
...
public:
	...
	CGeoPoint getPosForFifthVehicle() { return runPos[4]; }
	CGeoPoint getPosForSixthVehicle() { return runPos[5]; }
	CGeoPoint getPosForAssister() { return assisterPos; }
	CGeoPoint getPosForGoalie() { return runPos[6]; }
	...
	//
	void numberReceive(int first, int second, int third, int fourth, int fifth, int sixth, int assister, int goalie);
    void receiveOwnPos(std::vector <CGeoPoint> myPos  , int number);
	void changeLeaderOver(bool finishFlag);
    std::vector <CGeoPoint> generateShootPos();
    int chipPower;
	enum {
		CornerKick = 1,
		FrontKick,
	...
	int number[Param::Field::MAX_PLAYER+1]; // modified by ftq
	CGeoPoint runPos[7]; 		// 数字等于场上球员数目-1 （减掉了Assister)
	CGeoPoint confirmPos[7];	// 同上
	CGeoPoint shootPos;
    CGeoPoint assisterPos;
	const CGeoRectangle* getRunArea();
    void generateRunPos(const int areaNumber,std::vector<CGeoPoint>& myPos);
...
private:
	...
	int getBallNumber;
	int ramdomNumber[12];			// 数字等于场上球员数目-1 （减掉了Assister)
	int buf[Param::Field::MAX_PLAYER+1];				// max
	int maxDefendKickbuf;
	int maxbuf;
	int state[Param::Field::MAX_PLAYER+1];				// 啥玩意 by ftq
	int attempts;
	...
	int determinCycle;
	int pickNumber;
    std::vector <bool> flatPassAngleFlag;
    std::vector <double> flatPassAngle;
    std::vector <CGeoPoint> luaPos;
	double deltaAngle;
	int _cycle1;
	int _cycle2;
	bool detectOverFlag;
...
public:
	/**
	@brief	开启线程所用的函数*/
    static void ZSS_THREAD_FUNCTION doBestCalculation();
private:
